...
namespace SistemaAcademico.AcademicProgress.Core.Services
{
    ...
    public class AcademicProgressService : IAcademicProgressService
    {
        ...
        /// <inheritdoc />
        public async Task<AcademicIndexDto?> GetAcademicIndicesAsync(Guid estudianteId)
        {
            if (estudianteId == Guid.Empty)
            {
                throw new ArgumentException("El ID del estudiante no puede ser vacío.", nameof(estudianteId));
            }
